...
pi_0 <- 0 # loading sparsity options: 0.9, 0.3, 0
tau <- 1/25 # corresponsing noise precision options: 1, 1/16, 1/25
...
ggsave(filename = "figure_LF3.png", path = "output/figures", plot = figure_LF, height = 5, width = 5)

figure_LF_flash <- plot_matrix(flash_get_lf(flash_v2_out))
ggsave(filename = "figure_LF_flash3.png", path = "output/figures", plot = figure_LF_flash, height = 5, width = 5)

figure_flash_reconstruct_error <- plot_matrix(LF - flash_get_lf(flash_v2_out))
ggsave(filename = "figure_flash_reconstruct_error3.png", path = "output/figures", plot = figure_flash_reconstruct_error, height = 5, width = 5)

figure_PMD_reconstruct_error <- plot_matrix(LF - PMD_out$d * PMD_out$u  %*% t(PMD_out$v))
ggsave(filename = "figure_PMD_reconstruct_error3.png", path = "output/figures", plot = figure_PMD_reconstruct_error, height = 5, width = 5)

figure_SSVD_reconstruct_error <- plot_matrix(LF - SSVD_out$d * SSVD_out$u  %*% t(SSVD_out$v))
ggsave(filename = "figure_SSVD_reconstruct_error3.png", path = "output/figures", plot = figure_SSVD_reconstruct_error, height = 5, width = 5)
