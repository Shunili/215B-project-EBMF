...
pi_0 <- 0.3 # loading sparsity options: 0.9, 0.3, 0
tau <- 1/16 # corresponsing noise precision options: 1, 1/16, 1/25
...
for (i in 1:num_sim) {
  ...
  sample <- matrix(rnorm(n = 5*n, mean = mus[rep(1:5, each = n)], sd = sqrt(sigma_squared[rep(1:5, each = n)])), nrow = n)
  LL <- pi_0 + (1 - pi_0) * 1/5 * rowSums(sample)
...
#                 PMD2 = PMD2_error,
                 ...
                 #SVD = SVD_error)
long <- melt(df)
write.csv(long, "output/csv_files/toy1.csv")

toy1 <- ggplot(data = long) + 
  geom_boxplot(aes(y = value, color = variable)) + scale_y_sqrt()
ggsave(filename = "toy1.png", path = "output/figures", plot = toy1, height = 5, width = 5)



# potentially useful plots
figure_LF <- plot_matrix(LF)
ggsave(filename = "figure_LF.png", path = "output/figures", plot = figure_LF, height = 5, width = 5)

figure_LF_flash <- plot_matrix(flash_get_lf(flash_v2_out))
ggsave(filename = "figure_LF_flash.png", path = "output/figures", plot = figure_LF_flash, height = 5, width = 5)

figure_flash_reconstruct_error <- plot_matrix(LF - flash_get_lf(flash_v2_out))
ggsave(filename = "figure_flash_reconstruct_error.png", path = "output/figures", plot = figure_flash_reconstruct_error, height = 5, width = 5)

figure_PMD_reconstruct_error <- plot_matrix(LF - PMD_out$d * PMD_out$u  %*% t(PMD_out$v))
ggsave(filename = "figure_PMD_reconstruct_error.png", path = "output/figures", plot = figure_PMD_reconstruct_error, height = 5, width = 5)

figure_SSVD_reconstruct_error <- plot_matrix(LF - SSVD_out$d * SSVD_out$u  %*% t(SSVD_out$v))
ggsave(filename = "figure_SSVD_reconstruct_error.png", path = "output/figures", plot = figure_SSVD_reconstruct_error, height = 5, width = 5)
